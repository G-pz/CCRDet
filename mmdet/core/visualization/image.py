...
__all__ = [
    ...
    'imshow_det_bboxes', 'imshow_gt_det_bboxes', 'imshow_gt_det_difference_bboxes', 'imshow_gt_bboxes'
]
...
def imshow_gt_det_bboxes(img,
    ...
    return img
def imshow_gt_det_difference_bboxes(img,
                         annotation,
                         result,
                         class_names=None,
                         score_thr=0,
    ...
    labels = np.concatenate(labels)
    gt_labels = np.concatenate((gt_labels, labels), axis=0)
    bboxes_thr = []
    for i in range(bboxes.shape[0]):
        if bboxes[i, -1] > score_thr:
            if i == 0:
                bboxes_thr = bboxes[0, :-1]
                # print(type(bboxes_thr))
            elif type(bboxes_thr) is not list:
                bboxes_thr = np.vstack((bboxes_thr, bboxes[i, :-1]))

    # print(type(bboxes_thr))
    bboxes_TP_gt = []
    bboxes_TP_det = []
    # print(bboxes_thr.shape)
    if type(bboxes_thr) is not list:
        if bboxes_thr.ndim == 1:
            bboxes_thr = np.atleast_2d(bboxes_thr)
        # print(bboxes_thr.shape)
        ious = bbox_overlaps(gt_bboxes, bboxes_thr)
        for i in range(ious.shape[0]):
            for j in range(ious.shape[1]):
                if ious[i, j] > 0.3:
                    bboxes_TP_det.append(bboxes_thr[j, :])
                    bboxes_TP_gt.append(gt_bboxes[i, :])

        TP_det_array = np.array(bboxes_TP_det)
        TP_gt_array = np.array(bboxes_TP_gt)

        mask = np.isin(bboxes_thr, TP_det_array).all(axis=1)
        bboxes_FP = bboxes_thr[~mask]
        mask = np.isin(gt_bboxes, TP_gt_array).all(axis=1)
        bboxes_FN = gt_bboxes[~mask]

    else:
        TP_det_array = bboxes
        bboxes_FP = np.array(0)
        bboxes_FN = gt_bboxes

    # TP_det_array = TP_det_array[:, :-1]
    TP_det_array = np.vstack((TP_det_array, bboxes_FN[0], bboxes_FP[2],bboxes_FP[1]))
    # bboxes_FN = np.vstack((bboxes_FN[0],bboxes_FN[1],bboxes_FN[4],bboxes_FN[5],bboxes_FN[6],bboxes_FN[7],bboxes_FN[8]))
    bboxes_FP = np.vstack((bboxes_FP[0],bboxes_FP[3]))
    # bboxes_FP = bboxes_FP[0]
    bboxes_FN = np.vstack((bboxes_FN[1],bboxes_FN[2]))
    if TP_det_array.ndim == 2:
        img = imshow_det_bboxes(
            img,
            TP_det_array,
            gt_labels,
